...
class KontaktService(internal val repository: KontaktRepository, internal val personService: PersonService) {
    fun flyttKontaktTilGeneriskTidligereMedlem(
        tidligereMedlem: Int?,
        personId: Int
    ...
    fun hentKontakt(id: Int) =
        repository.list("frivillig_id", id).map { i ->
            KontaktResponse(
    ...
    fun registrerKontakt(kontakt: AutentisertRegistrerKontaktRequest) =
        repository.persist(
            Kontakt(
                frivillig_id = kontakt.request.frivillig_id,
                tilbakemelding = kontakt.request.tilbakemelding,
                registrert_av =
                    personService.finnFraHypersysId(kontakt.userId.userId)
