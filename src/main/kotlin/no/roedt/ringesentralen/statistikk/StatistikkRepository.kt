...
class StatistikkRepository(internal val entityManager: EntityManager) {
    ...
    fun getPersonerStatistikkResponse(): PersonerStatistikkResponse =
        PersonerStatistikkResponse(
            antallPersonerISystemetTotalt = entityManager.list("SELECT 1 FROM person").size,
            ringere =
                entityManager.list(
                    "SELECT 1 FROM person WHERE groupID in (" +
            ...
            ferdigringte =
                entityManager.list(
                    "select 1 FROM person WHERE " +
                        "groupID=${RingesentralenGroupID.Ferdigringt.nr} " +
                        "or groupID=${RingesentralenGroupID.Slett.nr}"
                ).size,
            ringtUtenSvar =
                entityManager.list(
                    "select 1 FROM person p inner join samtale s on s.ringt=p.id " +
                        "AND p.groupID=${RingesentralenGroupID.KlarTilAaRinges.nr}"
                ).size,
            ikkeRingt =
                entityManager.list(
                    "select 1 FROM person p where p.groupID < ${RingesentralenGroupID.UgodkjentRinger.nr} " +
                        "and not exists (select 1 from samtale s where s.ringt=p.id)"
                ).size,
    ...
    internal fun mineRingte(hypersysId: Int): Int =
        entityManager.list(
    ...
    internal fun personSomHarRingtFlest(): Int =
        entityManager.list(
