...
class HistorikkRepository(internal val entityManager: EntityManager) {
    ...
    fun hentMineSamtaler(userId: UserId, modus: Modus) = entityManager.list(
    ...
    fun hentLagetsSamtaler(modus: Modus, lokallag: Int) = entityManager.list(
        "select resultat, ringerNavn, tidspunkt, kommentar, oppringtNummer, ringtNavn, oppfoelgingId " +
            "from v_mineSamtaler where lokallag='$lokallag' and modus='${modus.name}'"
    )

    fun tellMineSamtaler(userId: UserId) =
        entityManager.list("select count(1) from samtale s inner join ringer r on s.ringer=r.id inner join person p on p.id=r.personId where hypersysID='${userId.userId}'")
}
