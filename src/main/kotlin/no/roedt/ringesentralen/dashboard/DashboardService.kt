...
class DashboardServiceBean(
    ...
    override fun getDashboard(ringerID: UserId): DashboardResponse {
        ...
        val statusliste: List<Lokallagsstatus> = mineLokallag
                .map { lokallag ->
                    val igjenAaRinge = personRepository.list("groupID<2 and lokallag=?1", lokallag.id).size
                    ...
                    val totaltInklRingte = personRepository.list("lokallag=?1 and groupID=2", lokallag.id).size
                    Lokallagsstatus(
                            lokallag = lokallag,
