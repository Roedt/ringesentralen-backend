...
data class RealFirestoreCollection(override val underforumnavn: String, val collectionReference: CollectionReference) :
    FirestoreCollection {
    override fun listDocuments(): Iterable<FirestoreDocument> =
...
private fun DocumentReference.tilFirestoreDocument(): FirestoreDocument =
    RealFirestoreDocument(
        tittel = this.id,
        underforum = this.parent.path,
        firestoreReferanse = this
    )

class FakeFirestoreCollection(override val underforumnavn: String, private val dokumenter: Iterable<FirestoreDocument>) :
    FirestoreCollection {
